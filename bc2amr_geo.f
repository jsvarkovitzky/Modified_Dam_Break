...
c        # outside the physical boundry.
...
c     # Use undisturbed state h0 = 10.0 cm.
      ...
      speed = dsqrt(grav*h0)
      eta1 = 0.218d0             ! depth of water at rest in the wavetank
      t1 = t + 0.5d0*hx / speed  ! ghost cell adjecent to boundary
      t2 = t + 1.5d0*hx / speed  ! next ghost cell away from boundary

      do it = 1,202
         if (profile(it,1).lt.t1.and.profile(it+1,1).ge.t1)then
            h1 = profile(it,2) + 
     &      (t1-profile(it,1))*((profile(it+1,2)-profile(it,2))/
     &      (profile(it+1,1)-profile(it,1)))
        endif
         if (profile(it,1).lt.t2.and.profile(it+1,1).ge.t2)then
            h2 = profile(it,2) + 
     &      (t2-profile(it,1))*((profile(it+1,2)-profile(it,2))/
     &      (profile(it+1,1)-profile(it,1)))
        endif
...
c      do it=1,201
c         if (profile(it,1).le.t1.and.profile(it+1,1).gt.t1) then
c            etaslope1=(profile(it+1,2)-profile(it,2))/.028975d0
c            eta1=profile(it,2)+ etaslope1*(t1-profile(it,1))
cc         print *, "pro(1): ", profile(it,1),"pro(2): ", profile(it,2)
c            do it2=it,201
c                if (profile(it2,1).le.t2.and.
c     &                    profile(it2+1,1).gt.t2) then
c                   etaslope2=(profile(it2+1,2)-profile(it2,2))/.028975d0
c                   eta2=profile(it2,2)+ etaslope2*(t2-profile(it2,1))
c                   go to 101
c                 endif
c               enddo
c               
...
c      write(*,*) 't2 is out of range',profile(it2,1),t2,profile(it2+1,1)
c      stop
 101  continue
