...
c       This subtracts away the water depth from the gague data
        eta = topo + var(1) - 0.218
c       This scaling effects the magnitude of the tide gauge
        eta = eta*1.0e0
c       This shifts the time of the gauge data
        tgrid = tgrid + 0

        do j=1,3
            if (abs(var(j)) .lt. 1.d-90) var(j) = 0.d0
         enddo
